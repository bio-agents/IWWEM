/*
	...
	*	Spanish National Bioinformatics Institute (INB, http://www.inab.org/)
	...
	This derivated code is part of IWWE&M, the Interactive Web Workflow Enactor & Manager.
...
*/
/* This modified code is based on two original files (see licenses below) */
/**
 * @fileoverview
 * 
...
1.2.1 (2007-01-09)
fixed an issue in the method .updateAgenttip() for cases where an element did not have a agenttip text, attribute or an empty string. Now, the agenttip disappears for these elements instead of triggering a javascript error.
...
function SVGmapApp(/*optional*/thedoc,adjustVBonWindowResize,resizeCallbackFunction) {
	...
	this.resetFactors();
	//per default, agenttips are disabled
	this.agenttipsEnabled = false;
...
SVGmapApp.Helpers = {
	 ...
	 */
	getTransformToRootElement: function (node,/*optional*/thedoc,theDocumentElement) {
		if(!theDocumentElement) {
			if(!thedoc)  thedoc=document;
			theDocumentElement=thedoc.documentElement;
		}
		try {
			//this part is for fully conformant players (like Opera, Batik, Firefox, Safari ...)
			var CTM = node.getTransformToElement(theDocumentElement);
		} catch (ex) {
...
SVGmapApp.prototype = {
	handleEvent: function(evt) {
		...
		if ((evt.type == "mouseover" || evt.type == "mouseout" || evt.type == "mousemove") && this.agenttipsEnabled) {
			this.displayAgenttip(evt);
		}
	},
	...
	//initialize tootlips
	initAgenttips: function(groupId,agenttipTextAttribs,agenttipRectAttribs,xOffset,yOffset,padding) {
		var nrArguments = 6;
		if (arguments.length == nrArguments) {
			this.agentTipGroup = this.thedoc.getElementById(groupId);
			this.agenttipTextAttribs = agenttipTextAttribs;
			if (!this.agenttipTextAttribs["font-size"]) {
				this.agenttipTextAttribs["font-size"] = 12;
			}	
			this.agenttipRectAttribs = agenttipRectAttribs;
			this.xOffset = xOffset;
			this.yOffset = yOffset;
			this.padding = padding;
			if (!this.agentTipGroup) {
				alert("Error: could not find agenttip group with id '"+groupId+"'. Please specify a correct agenttip parent group id!");
			} else {
				//set agenttip group to invisible
				this.agentTipGroup.setAttributeNS(null,"visibility","hidden");
				this.agentTipGroup.setAttributeNS(null,"pointer-events","none");
				this.agenttipsEnabled = true;
				//create agenttip text element
				this.agenttipText = this.thedoc.createElementNS(SVGmapApp.svgNS,"text");
				for (var attrib in this.agenttipTextAttribs) {
					value = this.agenttipTextAttribs[attrib];
					if (attrib == "font-size") {
						value += "px";
					}
					this.agenttipText.setAttributeNS(null,attrib,value);
				}
				//create textnode
				var textNode = this.thedoc.createTextNode("Agenttip");
				this.agenttipText.appendChild(textNode);
				this.agentTipGroup.appendChild(this.agenttipText);
				var bbox = this.agenttipText.getBBox();
				this.agenttipRect = this.thedoc.createElementNS(SVGmapApp.svgNS,"rect");
				this.agenttipRect.setAttributeNS(null,"x",bbox.x-this.padding);
				this.agenttipRect.setAttributeNS(null,"y",bbox.y-this.padding);
				this.agenttipRect.setAttributeNS(null,"width",bbox.width+this.padding*2);
				this.agenttipRect.setAttributeNS(null,"height",bbox.height+this.padding*2);
				for (var attrib in this.agenttipRectAttribs) {
					this.agenttipRect.setAttributeNS(null,attrib,this.agenttipRectAttribs[attrib]);
				}
				this.agentTipGroup.insertBefore(this.agenttipRect,this.agenttipText);
			}
		} else {
				alert("Error in method 'initAgenttips': wrong nr of arguments! You have to pass over "+nrArguments+" parameters.");			
		}
	},

	addAgenttip: function(agenttipNode,agenttipTextvalue,followmouse,checkForUpdates,targetOrCurrentTarget,childAttrib) {
		var nrArguments = 6;
		if (arguments.length == nrArguments) {
			//get reference
			if (typeof(agenttipNode) == "string") {
				agenttipNode = this.thedoc.getElementById(agenttipNode);
			}
			//check if agenttip attribute present or create one
			if (!agenttipNode.hasAttributeNS(SVGmapApp.attribNS,"agenttip")) {
				if (agenttipTextvalue) {
					agenttipNode.setAttributeNS(SVGmapApp.attribNS,"agenttip",agenttipTextvalue);
				}
				else {
					agenttipNode.setAttributeNS(SVGmapApp.attribNS,"agenttip","Agenttip");			
				}
			}
			//see if we need updates
			if (checkForUpdates) {
				agenttipNode.setAttributeNS(SVGmapApp.attribNS,"agenttipUpdates","true");		
			}
			//see if we have to use evt.target
			if (targetOrCurrentTarget == "target") {
				agenttipNode.setAttributeNS(SVGmapApp.attribNS,"agenttipParent","true");
			}
			//add childAttrib
			if (childAttrib) {
				agenttipNode.setAttributeNS(SVGmapApp.attribNS,"agenttipAttrib",childAttrib);
			}
			//add event listeners
			agenttipNode.addEventListener("mouseover",this,false);
			agenttipNode.addEventListener("mouseout",this,false);
			if (followmouse) {
				agenttipNode.addEventListener("mousemove",this,false);
			}
		}
		else {
			alert("Error in method 'addAgenttip()': wrong nr of arguments! You have to pass over "+nrArguments+" parameters.");			
		}
	},

	displayAgenttip: function(evt) {
		var curEl = evt.currentTarget;
		var coords = this.calcCoord(evt,this.agentTipGroup.parentNode);
		if (evt.type == "mouseover") {
			this.agentTipGroup.setAttributeNS(null,"visibility","visible");
			this.agentTipGroup.setAttributeNS(null,"transform","translate("+(coords.x+this.xOffset)+","+(coords.y+this.yOffset)+")");
			this.updateAgenttip(evt);
		}
		...
		if (evt.type == "mousemove") {
			this.agentTipGroup.setAttributeNS(null,"transform","translate("+(coords.x+this.xOffset)+","+(coords.y+this.yOffset)+")");		
			if (curEl.hasAttributeNS(SVGmapApp.attribNS,"agenttipUpdates")) {
				this.updateAgenttip(evt);
			}
		}
	},

	updateAgenttip: function(evt) {
		var el = evt.currentTarget;
		if (el.hasAttributeNS(SVGmapApp.attribNS,"agenttipParent")) {
			var attribName = "agenttip";
			if (el.hasAttributeNS(SVGmapApp.attribNS,"agenttipAttrib")) {
				attribName = el.getAttributeNS(SVGmapApp.attribNS,"agenttipAttrib");
			}
			el = evt.target;
			var myText = el.getAttributeNS(SVGmapApp.attribNS,attribName);
		}
		else {
			var myText = el.getAttributeNS(SVGmapApp.attribNS,"agenttip");
		}
		if (myText) {
			var textArray = myText.split("\\n");
			while(this.agenttipText.hasChildNodes()) {
				this.agenttipText.removeChild(this.agenttipText.lastChild);
			}
			for (var i=0;i<textArray.length;i++) {
				var tspanEl = this.thedoc.createElementNS(SVGmapApp.svgNS,"tspan");
				tspanEl.setAttributeNS(null,"x",0);
				var dy = this.agenttipTextAttribs["font-size"];
				if (i == 0) {
					var dy = 0;
				}
				...
				this.agenttipText.appendChild(tspanEl);
			}
	...
	enableAgenttips: function() {
		this.agenttipsEnabled = true;
	},

	disableAgenttips: function() {
		this.agenttipsEnabled = false;
		this.agentTipGroup.setAttributeNS(null,"visibility","hidden");
	}
};
